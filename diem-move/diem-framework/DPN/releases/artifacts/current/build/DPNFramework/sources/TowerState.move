...
module TowerState {
    ...
    public fun init_tower_counter(
      ...
      if (!exists<TowerCounter>(@VMReserved)) {
    ...
    public fun commit_state(
      ...
      // let diff = borrow_global<VDFDifficulty>(@VMReserved );
      ...
      if (!is_init(miner_addr)) {
        assert!(&proof.difficulty 
          == &Globals::get_vdf_difficulty_baseline(), Errors::invalid_argument(130102));
        assert!(&proof.security
          == &Globals::get_vdf_security_baseline(), Errors::invalid_argument(13010202));
    ...
    fun epoch_param_reset(vm: &signer) acquires VDFDifficulty, TowerList, TowerProofHistory  {
      ...
      let diff = borrow_global_mut<VDFDifficulty>(@VMReserved );
    ...
    fun increment_stats(miner_addr: address) acquires TowerProofHistory, TowerCounter {
      ...
      if (!exists<TowerCounter>(@VMReserved)) return;

      let above = node_above_thresh(miner_addr);

      let state = borrow_global_mut<TowerCounter>(@VMReserved);
    ...
    public fun get_validator_proofs_in_epoch(): u64 acquires TowerCounter{
      let state = borrow_global<TowerCounter>(@VMReserved);
    ...
    public fun get_lifetime_proof_count(): (u64, u64, u64) acquires TowerCounter{
      let s = borrow_global<TowerCounter>(@VMReserved);
    ...
    public fun get_difficulty(): (u64, u64) acquires VDFDifficulty {
      if (exists<VDFDifficulty>(@VMReserved )) {
        let v = borrow_global_mut<VDFDifficulty>(@VMReserved );
    ...
    public fun test_helper_operator_submits(
      ...
      if (!Testnet::is_testnet()){ // todo: remove?
        assert!(&proof.difficulty
          == &Globals::get_vdf_difficulty_baseline(), Errors::invalid_state(130117));
      };
    ...
    public fun test_set_vdf_difficulty(vm: &signer, diff: u64, sec: u64) acquires VDFDifficulty {
      ...
      let s = borrow_global_mut<VDFDifficulty>(@VMReserved );
      s.difficulty = diff;
      s.security = sec;
    }    
    ...
    public fun test_danger_destroy_tower_counter(vm: &signer) acquires TowerCounter {
      assert!(Testnet::is_testnet(), Errors::invalid_state(130113));
      CoreAddresses::assert_vm(vm);
      assert!(exists<TowerCounter>(@VMReserved), Errors::invalid_state(130115));
       ...
       } = move_from<TowerCounter>(@VMReserved);
    }
}
}
